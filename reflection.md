...
The secret number keeps changing because st.session_state.secret is initialized multiple times per session. This is now prevented and the secret is initialized only once per session and ensured that it doesn't changing for the remianing of that specific gameplay.

Streamlit "reruns" or starts the process all over again, which may include rendering, happens when st.rerun is implemented. A rerun stops current run and executes the script from the top. However, session state values do not change when reruns are executed. This is why it is important to set session states before reruns to prevent unwanted game characteristics. Local variables do not persist because they are created on each run.
